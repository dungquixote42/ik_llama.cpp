...
bool gpt_params_find_arg(int argc, char ** argv, const std::string & arg, gpt_params & params, int & i, bool & invalid_param) {
    ...
    if (arg == "--adaptive-ratio") {
        CHECK_ARG
        sparams.adaptive_ratio = std::stof(argv[i]);
        return true;
    }
    if (arg == "--spec-replace") {
        CHECK_ARG
...
void gpt_params_print_usage(int /*argc*/, char ** argv, const gpt_params & params) {
    ...
    options.push_back({ "*",           "       --adaptive-ratio",       "adaptive-p sampling: (default: %.2f)", (double)sparams.adaptive_ratio});
...
void yaml_dump_non_result_info(FILE * stream, const gpt_params & params, const llama_context * lctx,
    ...
    fprintf(stream, "adaptive_ratio: %f # default: 1.0\n", sparams.adaptive_ratio);
